...
#define USB_MAX_STRING_WITH_NULL_LEN	(USB_MAX_STRING_WITH_NULL_LEN+1)
static int usb_string_copy(const char *s, char **s_copy)
{
	...
	char *copy = *s_copy;
	ret = strlen(s);
	if (ret > USB_MAX_STRING_WITH_NULL_LEN)
		return -EOVERFLOW;
...
{									\
	if (size >= USB_MAX_STRING_WITH_NULL_LEN)					\
		return -EINVAL;						\
	pr_info("%s %s len=%zu\n", __func__, buf, size); \
	return strlcpy(buffer, buf, USB_MAX_STRING_WITH_NULL_LEN); \
}									\
